...
class Message(object):
    ...
    def apply_shift(self, shift):
        ...
        charList = []
        for char in message:
            if char in encrypting_dict:
                charList.append(encrypting_dict[char])
            else:
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for i in range(26):
            wordMatches = 0
            guessedMessage = self.apply_shift(i)
            for word in guessedMessage.split(" "):
                if is_word(self.get_valid_words(), word):
                    wordMatches += 1
            if wordMatches > bestWordMatches:
                bestDecryption = (i, guessedMessage)
